...
/* C++14 聚合初始化 (按 struct persistent_config 声明顺序) */
static const struct persistent_config factory_default = {
	CONFIG_MAGIC,      /* magic */
	CONFIG_VERSION,    /* version */
	0,                 /* crc32 (computed by config_store_save) */
	DEVICE_GROUP_ID,   /* group_id */
	DEVICE_TYPE,       /* device_type */
	DEVICE_HUB_TYPE,   /* hub_type */
	DEVICE_ROOM_ID,    /* room_id */
	{0},               /* reserved[8] */
};
...
int config_store_init(void) {
	...
	NRF_LOG_INFO("Config initialized from factory defaults (group=0x%02X room=%d)",
...
int config_store_save(void) {
	...
	NRF_LOG_INFO("Config saved (group=0x%02X)", current_config.group_id);
	return 0;
}
...
int config_store_factory_reset(void) {
	...
	current_config.crc32 = config_crc(&current_config);
	config_valid = true;
	NRF_LOG_INFO("Factory reset complete");
	return 0;
}
