...
int config_store_init(void) {
	...
	SEGGER_RTT_printf(0, "[INFO] Config initialized from factory defaults (group=0x%02X room=%d)\n",
...
int config_store_save(void) {
	...
	SEGGER_RTT_printf(0, "[INFO] Config saved (group=0x%02X)\n", current_config.group_id);
	return 0;
}
