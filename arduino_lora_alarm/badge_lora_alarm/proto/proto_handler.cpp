 ...
 * 移植自 NCS: 移除 Zephyr/logging/include, LOG→LOG_*
 */
#include <Arduino.h>
#include <string.h>
...
uint8_t proto_alarm_to_internal(uint8_t proto_val) {
	static const uint8_t map[8] = {
		[0] = ALARM_TYPE_GREEN, [1] = ALARM_TYPE_BLUE, [2] = ALARM_TYPE_YELLOW,
		[3] = ALARM_TYPE_RED,   [4] = ALARM_TYPE_HOLD, [5] = ALARM_TYPE_SECURE,
		[6] = ALARM_TYPE_EVACUATE, [7] = ALARM_TYPE_SHELTER,
	};
	return (proto_val < 8) ? map[proto_val] : 0xFF;
}

uint8_t internal_button_to_proto(uint8_t btn) {
	static const uint8_t map[4] = {[3] = 0, [1] = 1, [2] = 2, [0] = 3};
...
static int handle_code(const uint8_t *data, uint8_t len) {
	if (len < 2) { LOG_WARN(TAG, "CMD 0x03: need >=2 bytes"); return -22; }
	...
	if (alarm_type == 0xFF) {
		LOG_WARN(TAG, "CMD 0x03: unknown proto alarm %d", proto_alarm);
		return -22;
	}

	LOG_INFO(TAG, "CMD 0x03 Code: group=0x%02X alarm=%d -> internal=%d",
...
static int handle_code_setting(const uint8_t *data, uint8_t len) {
	if (len < 24) { LOG_WARN(TAG, "CMD 0x04: need 24 bytes"); return -22; }
	...
	LOG_INFO(TAG, "CMD 0x04: config updated alarm=%d prio=%d led=(%d,%d,%d) vol=%d",
...
static int handle_led_control(const uint8_t *data, uint8_t len) {
	if (len < 15) { LOG_WARN(TAG, "CMD 0x05: need 15 bytes"); return -22; }
...
static int handle_buzzer_control(const uint8_t *data, uint8_t len) {
	if (len < 12) { LOG_WARN(TAG, "CMD 0x06: need 12 bytes"); return -22; }
...
static int handle_clear_packet(const uint8_t *data, uint8_t len) {
	if (len < 2) { LOG_WARN(TAG, "CMD 0x0A: need 2 bytes"); return -22; }
	...
	if (clear_type == 0) {
		LOG_INFO(TAG, "CMD 0x0A: Clear All");
		alarm_sm_clear_all();
	} else if (clear_type == 1) {
		LOG_INFO(TAG, "CMD 0x0A: All Clear (Code Red only)");
		alarm_sm_all_clear();
	} else {
		return -22;
	}
...
static int handle_set_group_id(const uint8_t *data, uint8_t len) {
	if (len < 1) { LOG_WARN(TAG, "CMD 0x50: need >=1 byte"); return -22; }

	uint8_t new_group = data[0];
	LOG_INFO(TAG, "CMD 0x50: Set Group ID: 0x%02X -> 0x%02X",
...
/* ── 命令分发表 ── */
const cmd_handler_t cmd_handlers[256] = {
	[CMDID_CODE]               = handle_code,
	[CMDID_CODE_SETTING]       = handle_code_setting,
	[CMDID_LED_CONTROL]        = handle_led_control,
	[CMDID_BUZZER_CONTROL]     = handle_buzzer_control,
	[CMDID_VIBRATION_CONTROL]  = handle_vibration_control,
	[CMDID_LCD_CONTENT]        = handle_lcd_content,
	[CMDID_LCD_LINE2_ONOFF]    = handle_lcd_line2_onoff,
	[CMDID_CLEAR_PACKET]       = handle_clear_packet,
	[CMDID_SET_GROUP_ID]       = handle_set_group_id,
};
